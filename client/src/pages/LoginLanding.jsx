  ...
  const portalOptions = [
    {
      ...
      title: "Admin Portal",
      description:
        "Manage employees, departements, payroll and system configurations.",
      icon: ShieldIcon,
    },
    {
      to: "/login/employee",
      ...
      >
        <div className="w-full max-w-md animate-fade-in relative z-10">
          ...
          <div className="space-y-4">
            {portalOptions.map((item, index) => (
              <Link
                to={item.to}
                key={index}
                className="group block bg-slate-50 border border-slate-200 rounded-lg p-5
                sm:p-6 transition-all duration-300 hover:border-indigo-400 hover:bg-indigo-50"
              >
                <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                  <h3
                    className="text-lg text-slate-800 group-hover:text-indigo-600 mb-1
                  transition-colors"
                  >
                    {item.title}
                  </h3>
          ...
          <div className="mt-12 text-center md:text-left text-sm text-slate-400">
            <p>@ {new Date().getFullYear()} - SOMULCI. All rights reserved</p>
          </div>
        </div>
      </div>
    </div>
