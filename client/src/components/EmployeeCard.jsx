...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
  const handleDelete = async () => {
    if (!confirm("Etes vous sur de supprimer ? ")) return;
  };
  return (
    <div className="group relative card card-hover overflow-hidden">
      ...
      <div className="absolute top-3 left-3 flex gap-2">
        ...
        {employee.isDeleted && (
          <span className="bg-red-500/60 font-medium text-white px-2.5 py-1 rounded">
            DELETED
          </span>
        )}
      </div>
      ...
      {!employee.isDeleted && (
        ...
        >
          ...
          rounded-xl shadow-lg transition-all hover:scale-105 cursor-pointer"
          >
          ...
          rounded-xl shadow-lg transition-all hover:scale-105 disabled:opacity-50 cursor-pointer"
          >
